...
'''.replace('POLICY', policy)
    ...
    def test_native_pan_button_accepts_pencil_press(self):
        patch = (Path(__file__).resolve().parents[1] / 'patches/blender-ipad.patch').read_text(encoding='utf-8')
        section = patch.split('diff --git a/source/blender/editors/space_view3d/view3d_navigate_view_move.cc ', 1)[1].split('diff --git ', 1)[0]
        code = '\n'.join(line[1:] for line in section.splitlines()
                         if line.startswith((' ', '+')) and not line.startswith('+++'))
...
#ifdef WITH_APPLE_CROSSPLATFORM
    assert(invoke(&pan) == (fingers ? OPERATOR_RUNNING_MODAL : OPERATOR_FINISHED));
#else
